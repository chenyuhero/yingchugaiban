        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                if($row = $this->dsql->GetArray("al"))
                {
                    ...
                    if(preg_match('/j/', $row['flag']))
                    {
                    $row['filename'] = $row['arcurl'] = $row['keywords'];
                    }
        		else {
...
//                    $row['arcrank'],$row['namerule'],$row['typedir'],$row['money'],$row['filename'],$row['moresite'],$row['siteurl'],$row['sitepath']);
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a href='".$purl."PageNo=$prepagenum' class=\"prev\">上一页</a>\r\n";
            $indexpage="<a href='".$purl."PageNo=1' class=\"prev\">首页</a>\r\n";
        }
        else
        {
            $indexpage="<a class=\"prev\">首页</a>\r\n";
        }
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a href='".$purl."PageNo=$nextpagenum' class=\"prev\">下一页</a>\r\n";
            $endpage="<a href='".$purl."PageNo=$totalpage' class=\"prev\">末页</a>\r\n";
        }
        else
        {
            $endpage="<a class=\"prev\">末页</a>\r\n";
        }
        ...
        for($j;$j<=$total_list;$j++)
        {
            if($j==$this->PageNo)
            {
                $listdd.= "<a class=\"curr\">$j</a>\r\n";
            }
            else
            {
                $listdd.="<a href='".$purl."PageNo=$j' class=\"next\">".$j."</a>\r\n";
            }
        }
